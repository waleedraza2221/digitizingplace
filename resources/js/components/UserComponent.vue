  ...
  methods: {
    uploadPhoto(item) {
      if (this.editedItem.photo != null) {
        ...
        axios
          .post(this.path+"/user/photo", formData)
          .then(res => {
    ...
    updateRole(item) {
      const index = this.users.data.indexOf(item);
      axios
        .post(this.path+"/user/role", { role: item.role, user: item.id })
        .then(res => {
          this.text =
    ...
    checkEmail() {
      if (/.+@.+\..+/.test(this.editedItem.email)) {
        axios
          .post(this.path+"email/verify", { email: this.editedItem.email })
          .then(res => {
    ...
    deleteAll() {
      ...
      if (decide) {
        axios
          .post(this.path+"/users/delete", { users: this.selected })
          .then(res => {
    ...
    paginate(e) {
      ...
      const orderBy =
        this.options.sortDesc.length > 0 && this.options.sortDesc[0]
          ...
          : "desc";
      axios
        .get(`${this.path}users?page=${e.page}`, {
          params: {
    ...
    deleteItem(item) {
      ...
      if (decide) {
        axios
          .delete(this.path+"users/" + item.id)
          .then(res => {
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        axios
          .put(this.path+"users/" + this.editedItem.id, this.editedItem)
          .then(res => {
        ...
        axios
          .post(this.path+"users", this.editedItem)
          .then(res => {
